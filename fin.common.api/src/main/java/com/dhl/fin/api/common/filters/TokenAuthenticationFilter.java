...
public class TokenAuthenticationFilter extends OncePerRequestFilterBase {
    ...
    @Autowired
    public TokenAuthenticationFilter(WebConfig webConfig, TokenService tokenService) {
        super(webConfig);
...
//                }
    ...
    public boolean checkAppIsForbidden() {
        ...
        if (!isSuperManager && StringUtil.isNotEmpty(apps) && apps.toLowerCase().contains(context)) {
            ...
            return !Arrays.stream(apps.split("&")).filter(p -> p.contains(context + "-")).map(p -> ";" + p.replace(context + "-", "").replace("-", ";") + ";").anyMatch(p -> p.contains(";" + uuid + ";") || p.contains(";" + activeCode + ";"));
